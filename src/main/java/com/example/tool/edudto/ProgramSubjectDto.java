...
import com.globits.education.dto.EquivalentSubjectDto;
import com.globits.education.dto.GroupSubjectProgramDto;
...
public class ProgramSubjectDto implements Serializable {
	...
	public ProgramSubjectDto(ProgramSubject ps) {
		if(ps != null) {
			this.tuitionCoefficient = ps.getTuitionCoefficient();
			this.feePerCredit = ps.getFeePerCredit();
			this.mark = ps.getMark();
			this.id = ps.getId();
			this.subjectType = ps.getSubjectType();
			this.semesterIndex = ps.getSemesterIndex();
			if (ps.getProgram() != null) {
				this.program = new EducationProgramDto();
				this.program.setCode(ps.getProgram().getCode());
				this.program.setName(ps.getProgram().getName());
				this.program.setId(ps.getProgram().getId());
			}
			this.semester = ps.getSemester();
			if (ps.getSubject() != null) {
				this.subject = new SubjectDto();
				this.setDisplaySubjectName(ps.getSubject().getSubjectName());
				this.subject.setId(ps.getSubject().getId());
				this.subject.setSubjectCode(ps.getSubject().getSubjectCode());
				this.subject.setSubjectName(ps.getSubject().getSubjectName());
				this.subject.setSubjectNameEng(ps.getSubject().getSubjectNameEng());
				this.subject.setNumberOfCredit(ps.getSubject().getNumberOfCredit());
			}
			if(ps.getPrerequiteSubjects()!=null && ps.getPrerequiteSubjects().size()>0) {
				this.setPrerequiteSubjects(new ArrayList<PrerequisiteSubjectDto>());
				for(PrerequisiteSubject pre:ps.getPrerequiteSubjects()) {
					this.getPrerequiteSubjects().add(new PrerequisiteSubjectDto(pre));
				}
			}
			if(ps.getEquivalentSubjects()!=null && ps.getEquivalentSubjects().size()>0) {
				this.setEquivalentSubjects(new ArrayList<EquivalentSubjectDto>());
				for(EquivalentSubject e:ps.getEquivalentSubjects()) {
					this.getEquivalentSubjects().add(new EquivalentSubjectDto(e));
				}
			}
			if(ps.getPrerequiteSubjects()!=null && ps.getPrerequiteSubjects().size()>0) {
				this.setChildren(new ArrayList<PrerequisiteSubjectDto>());
				for(PrerequisiteSubject pre:ps.getPrerequiteSubjects()) {
					this.getChildren().add(new PrerequisiteSubjectDto(pre));
				}
			}
			if(ps.getGroup()!=null) {
				
			}
			if(ps.getKnowledgeProgram() != null) {
				KnowledgeProgramDto dto = new KnowledgeProgramDto(ps.getKnowledgeProgram());
				this.setKnowledgeProgram(dto);
			}
			this.setRoot(true);
		}
	}
